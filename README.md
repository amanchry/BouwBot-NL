...
├─ requirements.txt
├─ generated/               # auto-created; exported GeoJSON files live here
├─ tools/
│  ├─ tool_registry.py      # call_tool(name,args) dispatcher
│  ├─ tool_specs.py         # geospatial_tools list (OpenAI tool schema)
│  └─ utrecht_buildings.py  # geopandas tools (buffer, height filter, etc.)
├─ templates/
│  └─ index.html
└─ static/
   ├─ app.js
   ├─ style.css
   └─ data/
      └─ utrecht.geojson    # Utrecht boundary for map display (optional)
```
